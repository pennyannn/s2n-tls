...
int main(int argc, char **argv)
{
    ...
    if (s2n_is_tls13_fully_supported()) {
        ...
        /* A hello retry handshake cannot change to the TLS12 state machine */
        {
            ...
            /* Step through the expected handshake messages until just before the server has read the second Client Hello */
            EXPECT_ERROR_WITH_ERRNO(s2n_negotiate_test_server_and_client_until_message(server_conn, client_conn, SERVER_HELLO), S2N_ERR_IO_BLOCKED);
            s2n_blocked_status blocked = S2N_NOT_BLOCKED;
